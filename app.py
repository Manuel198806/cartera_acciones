...
def dashboard_view(df: pd.DataFrame) -> None:
    ...
    if audit is not None:
        st.subheader("Strategy Detection Audit")
        if isinstance(audit, pd.DataFrame) and not audit.empty:
            st.dataframe(audit, use_container_width=True, hide_index=True)
        else:
            st.info("No se detectaron estrategias automáticas con las reglas actuales.")
...
def position_chart_view(df: pd.DataFrame) -> None:
    ...
    try:
        ...
        st.warning(f"No se pudo importar yfinance: {exc}. Instala dependencias con `pip install -r requirements.txt`.")
        return
